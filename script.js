...
function initializeScrollAnimations() {
    // Create intersection observer with multiple animation types
    const observerOptions = {
        threshold: 0.1,
    ...
    document.querySelectorAll('.service-card, .equipment-card').forEach(el => {
        el.classList.add('animate-element', 'fade-up');
        fadeUpObserver.observe(el);
    });

    document.querySelectorAll('.about-content, .section-header').forEach(el => {
        el.classList.add('animate-element', 'fade-left');
        fadeLeftObserver.observe(el);
    });

    document.querySelectorAll('.about-visual, .hero-visual').forEach(el => {
        el.classList.add('animate-element', 'fade-right');
        fadeRightObserver.observe(el);
    });

    document.querySelectorAll('.project-card, .stat-item').forEach(el => {
        el.classList.add('animate-element', 'scale-up');
    ...
    try {
        ...
        try {
            ...
            if (jobsResponse.ok) {
                ...
                jobs = jobsData.jobs || [];
                window.jobsData = jobs; // Make jobs data available globally for modal
            ...
            return `
                <div class="job-card" data-job-id="${job.id}">
                    ...
                    <div class="job-actions">
                        <button class="job-apply-btn" onclick="openJobApplicationModal('${job.id}')">
...
async function renderPublicEquipments() {
    ...
    try {
        ...
        // Generate HTML for new equipment with same styling as placeholders
...
// Real Server Integration - Simple Resume Upload
...
function showApplicationForm(jobId) {
    ...
    const existingModal = document.getElementById('jobDetailsModal') || document.getElementById('jobApplicationModal');
    if (existingModal) {
        existingModal.remove();
    }
    
    const modalContent = `
        <div class="modal-overlay" id="jobApplicationModal">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form id="jobApplicationForm" class="application-form">
                        ...
                        <div class="form-actions">
                            ...
                            <button type="button" class="btn btn-primary" onclick="submitJobApplication('${jobId}')">
                                <i class="fas fa-paper-plane"></i>
                                Submit Application
                            </button>
                        </div>
...
async function loadApplicants() {
    try {
        const response = await fetch('http://localhost:8787/applicants');
        ...
        if (applicants.length === 0) {
            applicantsGrid.innerHTML = '<div class="no-applicants-message">No applicants yet.</div>';
            return;
        }
        ...
        const applicantsHTML = applicants.map(applicant => {
            ...
            const appliedDate = new Date(applicant.appliedDate).toLocaleDateString();
            ...
            return `
                <div class="admin-applicant-card">
                    ...
                    <div class="admin-applicant-info">
                        ...
                        ${applicant.applicantPhone ? `<p><strong>Phone:</strong> ${applicant.applicantPhone}</p>` : ''}
                        <p><strong>Resume:</strong> ${applicant.resumeOriginalName}</p>
                    </div>
                    <div class="admin-applicant-actions">
                        <a href="http://localhost:8787/download?filename=${applicant.resumeStoredName}" 
        ...
        const applicantsGrid = document.getElementById('applicantsGrid');
        if (applicantsGrid) {
            applicantsGrid.innerHTML = '<div class="no-applicants-message">Error loading applicants. Make sure the upload server is running.</div>';
        }
    }
}
...
function openJobApplicationModal(jobId) {
    ...
    const titleElement = document.getElementById('jobApplicationTitle');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const closeJobApplicationModalBtn = document.getElementById('closeJobApplicationModal');
    ...
    if (jobApplicationForm) {
        jobApplicationForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Use primary server URL first, then fallback
                const serverUrl = 'http://127.0.0.1:8787/upload';
                ...
                if (response.ok) {
                    ...
                    // Show success message with applicant details
                    const successMsg = result.applicant ? 
                    ...
                    console.log('Duplicate application detected:', errorData);
                ...
                // Try fallback server URL if primary fails
                try {
                    console.log('Trying fallback server URL...');
                    const fallbackUrl = 'http://localhost:8787/upload';
                    ...
                    if (fallbackResponse.ok) {
                        ...
                        console.log('Application submitted successfully via fallback:', result);
                ...
                alert('Network error: Unable to connect to server. Please check that the upload server is running on port 8787, then try again.');
            } finally {
                // Always reset submission state and re-enable form
    ...
    if (closeJobApplicationModalBtn) {
        closeJobApplicationModalBtn.addEventListener('click', closeJobApplicationModal);
    }
    
    // Close modal when clicking outside
    const jobApplicationModal = document.getElementById('jobApplicationModal');
    if (jobApplicationModal) {
        jobApplicationModal.addEventListener('click', function(e) {
            if (e.target === this) {
                closeJobApplicationModal();
            }
        });
...
// Update the existing initializeModals function to include job application modal
function initializeJobApplicationModal() {
    const jobApplicationModal = document.getElementById('jobApplicationModal');
    const closeJobApplicationModal = document.getElementById('closeJobApplicationModal');
    
    if (closeJobApplicationModal && jobApplicationModal) {
        closeJobApplicationModal.onclick = function() {
            jobApplicationModal.classList.remove('active');
        };
    }
    
    if (jobApplicationModal) {
        jobApplicationModal.onclick = function(e) {
            if (e.target === this) this.classList.remove('active');
        };
    }
}
...
async function testServerConnection() {
    ...
    for (const url of serverUrls) {
        try {
            ...
            console.log(`❌ Server unreachable at ${url}:`, error.message);
        }
    }
    
    console.error('❌ Upload server is not reachable. Make sure it\'s running on port 8787');
    return false;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (resumeInput) {
        resumeInput.addEventListener('change', function(e) {
            const fileName = e.target.files[0]?.name || 'Click to upload your resume';
